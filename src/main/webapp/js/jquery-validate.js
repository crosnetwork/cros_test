(function(factory) {
    ...
    var RULES = {
       ...
       int_3:function(value,errorMsg){
        	//int(3)
        	var reg = /^(\d|[1-9][0-9]{1,2})$/;
            if(!reg.test(value)){
                return errorMsg;
            }
        },
        float_5_2:function(value,errorMsg){
        	//float(5,2)
    		var reg = /^(\d|[1-9]\d{1,4}|[0-9]\.{1}\d{1,2}|[1-9][0-9]{1,3}\.{1}\d{1,2})$/;
            if(!reg.test(value)){
                return errorMsg;
            }
        },	
    	float_18_2:function(value,errorMsg){
    		//float(18,2)
    		var reg = /^(\d|[1-9]\d{1,17}|[0-9]\.{1}\d{1,2}|[1-9][0-9]{1,16}\.{1}\d{1,2})$/;
            if(!reg.test(value)){
                 return errorMsg;
            }
    	},
        ...
        isTelPhone:function(value,errorMsg){
            ...
            var isSuccess = regex.test(value) || regex2.test(value);
            if(!isSuccess ){
                return errorMsg;
            }
        },
        isNots:function(value,errorMsg){
            //不能输入特殊字符% ' " ? # ; ,
        	var reg = /[\%|\'|\|\"|\?|\#|\;|\,\，\；\“\‘]+/;  //不能输入特殊字符% ' " ? # ; ,
            if(reg.test(value)){
                return errorMsg;
        ...
        isNoAz09:function(value,errorMsg){
            //请输入5-15个字母或数字
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
       ...
       // $context.removeClass('success tip error').addClass(_current+' '+_other).append('<span class="valid_message">' + msg + '</span>');
        $context.find("span.err_desc").text(msg);
    };
