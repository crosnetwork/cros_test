...
public class WeibeiMsgApiImpl implements IWeibeiMsgApi {
    ...
    public String contractQueryData(String params) {
        ...
        try {
            ...
            String agentUserId = json.has("agent_user_id") == true ? json.getString("agent_user_id")
                    : null;// 用户唯一ID
            String contractHash = json.has("contract_hash") == true
                    ? json.getString("contract_hash")
                    : null; // 合同hash(md5)
            ...
            String txId = json.has("tx_id") == true ? json.getString("tx_id") : null; // 区块交易记录号
            String contractId = json.has("sys_contract_no") == true
                    ? json.getString("sys_contract_no")
                    : null; // 抵押物唯一标示
            int pageNumber = json.has("pageNumber") == true ? json.getInt("pageNumber") : 1; //
            int pageSize = json.has("pageSize") == true ? json.getInt("pageSize") : 10; //

            DBCursor dbs = coprApiService.queryByConfigContract("TxPool", agentUserId, txId,
                    contractId, contractHashs, pageNumber, pageSize);
            try {
                ...
                if (dbs != null && dbs.size() > 0) {
                    ...
                    while (dbs.hasNext()) {
                        ...
                        JSONArray jArray = JSONArray.parseArray(map.get("out").toString());
                        System.out.println(jArray.getJSONObject(0).get("data"));
                        String datajson = jArray.getJSONObject(0).get("data").toString();
                        ...
                        int high = coprApiService
                                .queryBlockChainHigh(document.get("tx_id").toString());
    ...
    public String contractRegisterData(String params) {
        ...
        try {
            ...
            if (json.containsKey("loan_type") && json.containsKey("sys_contract_no")
                    && json.containsKey("signing_date") && json.containsKey("loan_limit")
                    && json.containsKey("contract_hash")) {
            ...
            DBCursor userInfo = coprApiService.queryForUid(agentUserId, "Wallet");
            try {
                if (userInfo.hasNext()) {
                    ...
                    userWalletAddress = userObj.get("userWalletAddress").toString();
                } else {
                    logger.info(" : new user========================================");
                    ECKey eckey = new ECKey();
                    userPublicKey = eckey.getPublicKeyAsHex();
                    userPrivateKey = eckey.getPrivateKeyAsHex();
                    userWalletAddress = eckey.toAddress(MainNetParams.get()).toBase58();
                    DBObject obj = new BasicDBObject();
                    obj.put("userPublicKey",
                            TripleDES.encrypt(userPublicKey.getBytes()).toString());
                    obj.put("userPrivateKey",
                            TripleDES.encrypt(userPrivateKey.getBytes()).toString());
            ...
            ECKey systemECKey = ECKey.fromPrivateAndPrecalculatedPublic(
                    Utils.HEX.decode(systemPrivateKey), Utils.HEX.decode(systemPublicKey));
            String systemWalletAddress = systemECKey.toAddress(MainNetParams.get()).toBase58();
            ...
            Document docData = new Document("agent_id", agentId).append("inventory_hash",
                    asList(contractHashs));
            ...
            // Create a document
            Document doc = new Document("ver", 1).append("vin_size", 1).append("vout_size", 1)
                    .append("tx_time", cal.getTimeInMillis())
                    .append("in",
                            asList(new Document()
                                    .append("prev_out", "00000000000000000000000000000000")
                                    .append("prev_n", -1)))
                    .append("out",
                            asList(new Document().append("value", 1)
                                    .append("scriptPubKey",
                                            "OP_DUP OP_HASH160 " + userWalletAddress
            ...
            String docHash = Utils.HEX.encode(Sha256Hash.hashTwice(doc.toJson().getBytes()));
            ...
            String docHash2 = Utils.HEX.encode(Sha256Hash.hashTwice(doc.toJson().getBytes()));
    ...
    public String inventoryQueryData(String params) {
        ...
        try {
            ...
            String agentUserId = json.has("agent_user_id") == true ? json.getString("agent_user_id")
                    : null;// 用户唯一ID
            String inventoryHash = json.has("inventory_hash") == true
                    ? json.getString("inventory_hash")
                    : null; // 作品hash(md5)
            ...
            DBCursor dbs = coprApiService.queryByConfigInventory("TxPool", agentUserId, txId,
                    inventoryId, inventoryHashs, pageNumber, pageSize);
            try {
                JSONArray jsonArray = new JSONArray();
                if (dbs != null && dbs.size() > 0) {
                    result.put("status", 200);
                    while (dbs.hasNext()) {
                        ...
                        jsonObject.put("tx_id", document.get("tx_id"));
                        jsonObject.put("tx_time", document.get("tx_time"));
                        jsonObject.put("height", coprApiService
                                .queryBlockChainHigh(document.get("tx_id").toString()));
                        jsonArray.add(jsonObject);
                    }
                    result.put("data", jsonArray);
    ...
    public String inventoryRegisterData(String params) {
        ...
        try {
            ...
            if (json.containsKey("agent_id") && json.containsKey("agent_user_id")
                    && json.containsKey("inventory_id") && json.containsKey("inventory_hash")) {
