...
import org.cros.blockchain.service.IWeibeiBlockChainService;
import org.cros.blockchain.util.MerkleTree;
...
public class WeibeiBlockChainServiceImpl implements IWeibeiBlockChainService {
    ...
    public void saveToblockChain() {
        /*
         * 业务逻辑 Step1： 查询txPool 集中 status=pending的数据 DBTx Step2： DBTx count/100 = 0 执行打包
         * = 1 分2次 执行打包 = n 分n+1次 执行打包 # 打包逻辑 1. 查询上一个区块的信息 校验 txpool区块上的tx值是否有效
         * cuurrent previous prev_block block_hash值 height height+1 2. block heard size
         * DBTx count block_hash docment 转 hash merkleroot 所有被打包docment tx_id值
         * bitcoin_tx 目前为空 3. 签名和hash值 找到的所有记录写入document的 "tx" 节点
         * 
         * 4. 将docment写入 blockchain 5. 修改txPool数据状态
         * 
         */
